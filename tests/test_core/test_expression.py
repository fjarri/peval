...
def check_peval_expression_bool(source, bindings, expected_value):
    ...
    check_peval_expression(
        source,
        bindings,
        expected_source=str(expected_value),
...
def test_simple_cases():
    ...
    check_peval_expression('b"abc"', dict(b=s), 'b"abc"', fully_evaluated=True, expected_value=s)
...
def test_function_call():
    ...
    check_peval_expression(
        "fn(x, y, z=a + 1)",
        dict(fn=fn_args_kwds, x=1, y=2, a=10),
        "__peval_temp_1",
        expected_temp_bindings=dict(__peval_temp_1=(1, 2, 11)),
        fully_evaluated=True,
        expected_value=(1, 2, 11),
    )
...
def test_list_comprehension():
    ...
    check_peval_expression("[x + 1 for x in range(a)]", dict(a=10), "[x + 1 for x in range(10)]")
...
def test_dict_comprehension():
    ...
    check_peval_expression("{x+1:x+2 for x in range(a)}", dict(a=2), "{x+1:x+2 for x in range(2)}")
...
def test_propagation_int():
    check_peval_expression("a * n + (m - 2) * (n + 1)", dict(n=5), "a * 5 + (m - 2) * 6")


def test_propagation_float():
    check_peval_expression("a * n + (m - 2) * (n + 1)", dict(n=5.0), "a * 5.0 + (m - 2) * 6.0")


def test_propagation_str():
    check_peval_expression("a + foo", dict(foo="bar"), "a + 'bar'")
...
def test_preferred_name():
    ...
    check_peval_expression("m * n", dict(m=Int(2)), "m * n")
...
def test_arithmetic():
    check_peval_expression("1 + 1", {}, "2", fully_evaluated=True, expected_value=2)
    check_peval_expression("1 + (1 * 67.0)", {}, "68.0", fully_evaluated=True, expected_value=68.0)
    check_peval_expression("1 / 2.0", {}, "0.5", fully_evaluated=True, expected_value=0.5)
    check_peval_expression("3 % 2", {}, "1", fully_evaluated=True, expected_value=1)
    check_peval_expression(
